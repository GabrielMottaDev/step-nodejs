...
function start() {
  ...
  io.on('connection', function (socket) {
    ...
    socket.on('cadastrar', function (nome, categoria, preco) {
      hasMaterial(nome, categoria, function (has) {
        if (!has) {
          addMaterial(nome, categoria, preco);
          socket.emit('success', nome);
        } else {
          socket.emit('error-already-exist', nome, categoria);
        }
      });
    });
  });
...
function createTable() {
  var sql = "CREATE TABLE IF NOT EXISTS `material` (";
  sql += "`id` INT NOT NULL AUTO_INCREMENT, ";
  sql += "`nome` NCHAR(40) NOT NULL, ";
  sql += "`categoria` NCHAR(40) NOT NULL, ";
  sql += "`preco` DOUBLE NOT NULL, ";
  sql += "PRIMARY KEY (ID)";
  sql += ");";
...
function hasMaterial(nome, categoria, callback) {
  var sql = "SELECT * FROM `material` WHERE `nome`='" + nome + "' AND `categoria`='" + categoria + "'";
  connection.query(sql, function (error, results, fields) {
    callback(results.length > 0);
  });
}

function addMaterial(nome, categoria, preco) {
  const sql = "INSERT INTO `material`" +
    "(`nome` ,`categoria`, `preco`) VALUES" +
    "('" + nome + "', '" + categoria + "', '" + preco + "');";
  connection.query(sql, function (error, results, fields) {
    if (error) return console.log(error);
    console.log('Material ' + nome + ' cadastrado com sucesso!');
  });
};